/**
 * One-time SMS code issuance + verification for donor-portal logins
 * from an untrusted device.
 *
 * Flow:
 *   1. Donor enters PIN on a device with no `ttl_portal_device` cookie
 *      (or whose device was revoked).
 *   2. We call `issueOtp(scheduleId, phone)` which creates a PortalOtp row
 *      (5-minute TTL, SHA256-hashed code) and SMS's the 6-digit code.
...
/**
 * Request a new OTP. Rate-limited to 3/hour/phone so an attacker who knows
 * a valid PIN can't run up the Twilio bill. Returns `ok:false` if over
 * the limit; otherwise creates + sends. The function never reveals whether
 * the phone actually belongs to a real schedule — the caller is
 * responsible for verifying the PIN before calling.
 */
export async function issueOtp(scheduleId: string, phone: string): Promise<{ ok: boolean; reason?: string }> {
  const gate = rateLimit(`portal-otp:${phone}`, 3, 60 * 60 * 1000);
  ...
  const body =
    `Your TrueTest Labs portal code is ${code}. It expires in 5 minutes. ` +
